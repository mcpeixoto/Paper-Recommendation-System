...
for search_term in tqdm(search_terms, desc="Search Terms", leave=False, position=0, total=len(search_terms)):
    ...
    # Get the papers
    client = arxiv.Client(page_size=page_size, delay_seconds=delay_seconds, num_retries=num_retries)  # 2000 is the max  # arXiv's Terms of Use ask that you "make no more than one request every three seconds."
